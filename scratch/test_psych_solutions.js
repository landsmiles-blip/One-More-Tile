 ...
 * Standalone Verification Engine & Adversarial QA Rig
 */
...
// Exact cell enum integers as mandated:
...
class CognitiveSimEngine {
  ...
  move(dirName) {
    ...
    if (depCell === C_CRUMBLING) {
      this.grid[this.player.y][this.player.x] = C_VOID; // Collapses into abyss (0)
...
// ============================================================================
console.log("================================================================================");
console.log("   ONE MORE TILE — THE 10 COGNITIVE TRIALS (LEVELS 11-20) VERIFICATION RIG");
console.log("================================================================================\n");
...
levels.forEach((lvl, idx) => {
  ...
  assert.strictEqual(lvl.trace.length, lvl.par, `Level ${lvl.id} trace length must strictly match par (${lvl.par})`);

  process.stdout.write(`[Trial ${lvl.id}] "${lvl.name}" (World ${lvl.world}, Par ${lvl.par}) ... `);

  const engine = new CognitiveSimEngine(lvl);
  const initialRemaining = engine.getRemainingCount();
  assert.strictEqual(initialRemaining, lvl.par - 1, `Initial unconsumed tiles must equal par - 1`);

  // Execute trace step-by-step
  for (let step = 0; step < lvl.trace.length; step++) {
    const dir = lvl.trace[step];

    // Check pre-goal clearance right before final move
  ...
  // Verify Crossroad degradation: all crossroads must have 0 visits remaining
  ...
  passedCount++;
  console.log(`\x1b[32mPASSED\x1b[0m (Topo Branching: ${lvl.branching_factor})`);
});

console.log("\n================================================================================");
console.log(`   VERIFICATION COMPLETE: \x1b[32m${passedCount}/10 COGNITIVE TRIALS PASSED (100% SUCCESS)\x1b[0m`);
console.log("================================================================================\n");
